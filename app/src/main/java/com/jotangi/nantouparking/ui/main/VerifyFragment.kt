...
class VerifyFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding?.textResendOption2?.setOnClickListener { verifyCodeValue = generateSixDigitCode().toString()
            mainViewModel.sendVerify(requireContext(), ChargeNoticeFragment.idValue, verifyCodeValue)
        }
    }
            ...
            result?.let {
                when (result.code) {
                    "0x0200" -> {
                        ...
                        object : CountDownTimer(30_000, 1000) { // 30 seconds, tick every 1 second
