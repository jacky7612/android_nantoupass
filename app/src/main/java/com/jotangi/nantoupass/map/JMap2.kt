package com.jotangi.zhudongparking.jackyModels.map
...
import com.jotangi.nantoupass.jackyModels.map.JMapData
import com.jotangi.nantoupass.ui.BaseFragment
import com.jotangi.nantoupass.ui.BaseWithBottomBarFragment
import com.jotangi.nantoupass.ui.charge.MarkerInfoBottomSheet
...
class JMap2(
    view: View, val feetBinding: ToolbarFeetBinding, val resource: Resources,
...
//            Glob.assign(jmap_parkings[Glob.map_position])
    ...
    private fun refreshMarkerAtLocation(
        input_charge: List<JMapData>,
        input_parking: List<JMapData>
    ) {
...
class JMapCharge2(
    view: View, val resource: Resources,
    ...
    init {
        Glob.activity = view.context as? Activity ?: throw IllegalStateException("Activity context is required")
...
//        }
    ...
    private fun onMapClick(latLng: LatLng) {
        ...
        selectedMarker = mMap.addMarker(
            MarkerOptions()
                .position(latLng)
                .title("Selected Location")
                .icon(BitmapDescriptorFactory.fromBitmap(getBitmapFromVectorDrawable(R.drawable.ic_charge_placeholder48)))
        ) // select :ic_mark_cyan
    ...
    override fun onMarkerClick(marker: Marker): Boolean {
        ...
        selectedMarker?.let { previousMarker ->
            Log.d("micCheckLP", previousMarker.tag.toString())

            // Cast the tag to JChargeMapData and check the status
            val previousMarkerData = previousMarker.tag as? JChargeMapData
            val originalIconRes = when (previousMarkerData?.status) {
                0.toString() -> R.drawable.road_parking
                1.toString() -> R.drawable.road_parking2
                else -> null // Handle unknown or missing statuses
            }

            // Restore the original icon of the previously selected marker
            originalIconRes?.let { iconRes ->
        ...
        // Change the icon of the currently clicked marker to the highlighted state
        marker.setIcon(
            BitmapDescriptorFactory.fromBitmap(
                getBitmapFromVectorDrawable(R.drawable.ic_charge_placeholder48)
            )
        )

        // Update the selectedMarker to the currently clicked marker
        selectedMarker = marker
...
class JMapCharge3(
    view: View, val resource: Resources,
    val savedInstanceState: Bundle?,
    val jmap_parkings: List<JChargeMapData.JChargeMapData2>, val default_latlng_pos: Int,
    val mode: String,
    val ShowInfoInStart: Boolean
) :
    ...
    init {
        Glob.activity =
            view.context as? Activity ?: throw IllegalStateException("Activity context is required")
...
//        }
    ...
    override fun onMarkerClick(marker: Marker): Boolean {
        ...
        selectedMarker?.let { previousMarker ->
            ...
            previousMarker.setIcon(
                BitmapDescriptorFactory.fromBitmap(
                    getBitmapFromVectorDrawable(originalIconRes) // Ensure this method creates the correct bitmap
                )
            )
            }
    ...
    private fun addParkingSpots3() {
        ...
        jmap_cur.forEach { spot ->
            val iconRes = R.drawable.parking  // Unavailable
